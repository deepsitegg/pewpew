...
public class ItemsModule extends SpigotModule<PewpewPlugin> {
	...
	private static void validateKeys(@NotNull PewpewGunItem gun, @NotNull String fileName, @NotNull Logger log) {
		if (gun.getProjectileModel() != null && BukkitRegistry.item(gun.getProjectileModel()) == null)
			warnKey(log, fileName, gun, "projectileModel", gun.getProjectileModel());
		if (gun.getDamageType() != null && BukkitRegistry.damageType(gun.getDamageType()) == null)
			warnKey(log, fileName, gun, "damageType", gun.getDamageType());
		if (gun.getTrailParticle() != null && BukkitRegistry.particle(gun.getTrailParticle()) == null)
			warnKey(log, fileName, gun, "trailParticle", gun.getTrailParticle());
		if (gun.getImpactParticle() != null && BukkitRegistry.particle(gun.getImpactParticle()) == null)
			warnKey(log, fileName, gun, "impactParticle", gun.getImpactParticle());
		for (PewpewEffect effect : effectsOf(gun)) {
			if (BukkitRegistry.effectType(effect.type()) == null)
				warnKey(log, fileName, gun, "potion effect", effect.type());
		}
	}

	@NotNull
	private static List<PewpewEffect> effectsOf(@NotNull PewpewGunItem gun) {
		List<PewpewEffect> all = new ArrayList<>();
		if (gun.getVictimEffects() != null) all.addAll(gun.getVictimEffects());
		if (gun.getShooterEffects() != null) all.addAll(gun.getShooterEffects());
		return all;
	}

	private static void warnKey(Logger log, String fileName, PewpewGunItem gun, String field, Key key) {
		log.warning("[Pewpew] '" + gun.getId() + "' in " + fileName + ": unknown " + field
				+ " '" + key.asString() + "', it will be ignored");
	}
	...
	private void loadItems() {
		...
		for (var fileEntry : roots.entrySet()) {
			...
			for (PewPewItem item : items) {
				...
				register(item);
				if (item instanceof PewpewGunItem gun) validateKeys(gun, fileEntry.getKey(), log);
