...
public class ProjectileShotExecutor implements ShotExecutor {
	...
	public void execute(@NotNull Player shooter, @NotNull PewpewGunItem gun, @NotNull ItemStack weapon) {
		...
		for (int pellet = 0; pellet < pellets; pellet++) {
			...
			projectile.setGravity(gravity);
						Material projectileModel = BukkitRegistry.item(gun.getProjectileModel());
			if (projectileModel != null) projectile.setItem(new ItemStack(projectileModel));
	...
	private void trail(Entity projectile, Key particle) {
		...
		Bukkit.getScheduler().runTaskTimer(PewpewPlugin.getInstance(), task -> {
			...
			projectile.getWorld().spawnParticle(resolved, projectile.getLocation(), 1, 0, 0, 0, 0);
		}, 1L, 1L);
	}
