...
public class HitscanShotExecutor implements ShotExecutor {
	...
	private void fireBallistic(Player shooter, PewpewGunItem gun, Location eye, Vector direction, double range, double drop,
		...
		while (traveled < range) {
			...
			Particle trail = BukkitRegistry.particle(gun.getTrailParticle());
			if (trail != null) point.getWorld().spawnParticle(trail, point, 1, 0, 0, 0, 0);
	...
	private void spawnTracer(Location eye, Vector direction, double length, Key particle) {
		...
		for (double traveled = 1.5; traveled < length; traveled += 1.0) {
			eye.getWorld().spawnParticle(resolved, point, 1, 0, 0, 0, 0);
			point.add(direction);
		}
	}
}
