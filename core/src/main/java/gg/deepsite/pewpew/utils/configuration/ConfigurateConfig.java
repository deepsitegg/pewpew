...
public abstract class ConfigurateConfig {
	...
	public ConfigurateConfig(File file, String name, String def, boolean mergeDefaults) {
		...
		try {
			...
			PewpewLog.get().warning("An error occurred while loading this configuration: " + e.getMessage());
		}
	}
	...
	protected void migrate(@NotNull List<ConfigMigrator.Migration> migrations, int currentVersion, @NotNull String label) {
		...
		if (ConfigMigrator.migrate(rootNode, migrations, currentVersion, label,
				PewpewLog.get())) saveConfiguration();
	}
	...
	private void backup(int fromVersion) {
		...
		try {
			...
			PewpewLog.get().info("Backed up " + configFile.getName() + " to " + target.getName());
		} catch (IOException e) {
			PewpewLog.get()
	...
	public void saveConfiguration() {
		try {
			...
			PewpewLog.get().warning("Unable to save your messages configuration! Sorry! " + e.getMessage());
		}
	}
	...
	private void defaultConfig(File file, String name, String def, boolean mergeDefaults) {
		...
		if (!config.exists()) {
			try (InputStream resourceStream = ConfigurateConfig.class.getResourceAsStream("/" + def)) {
				if (resourceStream == null) {
					PewpewLog.get().warning("Could not find def resource: " + def);
					return;
				}
			...
			try (InputStream resourceStream = ConfigurateConfig.class.getResourceAsStream("/" + def)) {
				if (resourceStream == null) {
					PewpewLog.get().warning("Could not find def resource for merging: " + def);
					return;
				}
