...
public class RecoilController {
	...
	public RecoilController(@NotNull RotationSink sink, @NotNull RecoilProfile profile) {
		this.sink = sink;
	...
	public boolean tick() {
		...
		if (Math.abs(deltaYaw) > EPSILON || Math.abs(deltaPitch) > EPSILON) {
			if (!sink.applyRelative(deltaYaw, deltaPitch)) {
	...
	private void applyAbsolute(float deltaYaw, float deltaPitch) {
		float[] base = rebase(sink.yaw(), sink.pitch());
		float yaw = normalizeYaw(base[0] + deltaYaw);
		float pitch = Math.max(-90f, Math.min(90f, base[1] + deltaPitch));
		sink.setRotation(yaw, pitch);
	...
	float[] rebase(float observedYaw, float observedPitch) {
		...
		for (float[] previous : commanded) {
			float diffYaw = normalizeYaw(observedYaw - previous[0]);
